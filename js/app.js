...
function initFaceApi() {
    ...
    webcam.addEventListener('play', () => {
        ...
        setInterval(async () => {
            const detections = await faceapi.detectAllFaces(webcam, new faceapi.TinyFaceDetectorOptions()).withFaceLandmarks()
            const resizedDetections = faceapi.resizeResults(detections, displaySize)
            if (resizedDetections && resizedDetections[0].hasOwnProperty('landmarks')) {
                doDrag(resizedDetections[0].landmarks);
            }
        }, 100)
    })
